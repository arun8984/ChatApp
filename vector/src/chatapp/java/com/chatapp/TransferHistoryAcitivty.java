...
import com.chatapp.adapters.TransferHisAdapter;
import com.chatapp.adapters.TransferHisItem;
                        ...
                        if (!json.isNull("result")) {
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    try{
                                        //Toast.makeText(TransferHistoryAcitivty.this, json.getString("msg"), Toast.LENGTH_LONG).show();
                                        ArrayList<TransferHisItem> arrayList = new ArrayList<TransferHisItem>();
                                        for (int i =0; i<jsonArray.length();i++){
                                            JSONObject jsonObject = new JSONObject(jsonArray.get(i).toString());
                                            TransferHisItem transferItem = new TransferHisItem(jsonObject.getString("sender"),jsonObject.getString("reciever"),jsonObject.getString("amount"),jsonObject.getString("currency"),jsonObject.getString("date"));
                                            arrayList.add(transferItem);
                                        }
                                        if (arrayList.size() > 0) {
                                            listView.setVisibility(View.VISIBLE);
                                            mAdapter = new TransferHisAdapter(TransferHistoryAcitivty.this, R.layout.trans_item, arrayList);
